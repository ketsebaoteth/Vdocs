...
<template>
    <div :class="{'!my-1 border-l border-border' : props.isSub}" class="my-4 divide-y divide-border">
      <div v-for="(dir, index) in struct" :key="index" class="w-fit min-w-80">
        <div v-if="dir.isFolder">
          <div @click="toggleCollapse(index)" class="cursor-pointer bg-secondary rounded-md border-b border-border py-2 px-2 flex-place-center gap-2 transition-all">
            <IconFolder class="size-5 text-muted-foreground" />
            {{ dir.name }} 
        ...
        <div v-else class="py-2 px-2 flex-place-center gap-2">
